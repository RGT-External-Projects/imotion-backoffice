import * as React from 'react';
import type { IconProps } from './types';

/**
 * Sessions icon component based on the SVG you provided.
 */
const SessionsIcon: React.FC<IconProps> = ({
  size = 16,
  color = 'white',
  className,
}) => (
  <svg
    width={size}
    height={size}
    viewBox="0 0 16 16"
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
    className={className}
  >
    <path
      d="M12.85 4.65L9.35 1.15C9.25 1.05 9.15 1 9 1H4C3.45 1 3 1.45 3 2V14C3 14.55 3.45 15 4 15H12C12.55 15 13 14.55 13 14V5C13 4.85 12.95 4.75 12.85 4.65ZM9 2.2L11.8 5H9V2.2ZM12 14H4V2H8V5C8 5.55 8.45 6 9 6H12V14Z"
      fill={color}
    />
    <path d="M11 11H5V12H11V11Z" fill={color} />
    <path d="M11 8H5V9H11V8Z" fill={color} />
  </svg>
);

export default SessionsIcon;
